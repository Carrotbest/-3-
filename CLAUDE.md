...
## 방향
- DD MASTER = 현황 관리 중심. **DD MASTER와 창고는 엑셀을 파싱하지 않고 웹에서 직접 작성·관리한다(2026-09-14).** 엑셀은 백업과 문서 내보내기만. **엑셀 업로드는 쓰지 않는다(2026-09-22).** SETTING 파일 연결 센터와 DD 비상용, 샘플대장 업로드 경로를 없앴다(R241). 새 업로드 경로를 만들지 않는다.
...
- 목록에서 빼는 '선택 삭제'는 `REMOVED` 오버라이드로 감추는 것이다. 원본은 지우지 않는다. 폐기와 다르다. 감춘 행은 입고대기 탭 `숨긴 행 N건` 토글에서 보고 `되살리기`로 감추기 직전 상태로 돌린다(R231). DD MASTER 대장 상태 칸에는 `삭제됨`으로 뜬다. **사유가 남지 않으므로 개발을 접은 건은 감추지 말고 폐기(사유 `개발 중단`)로 보낸다**(2026-09-21). 2026-09-22부터 창고보관, 이력 표의 오른쪽 처리 열은 휴지통(같은 `REMOVE`) 하나만 있다. 재고, 출고, 소진, 폐기, 복구는 상단 선택 버튼으로 한다. 상단 `전체 확인 처리` 버튼은 없앴다(전체 선택 후 `입고 확인`). 번호를 받은 적 없는 건은 폐기해도 창고팀 자료에 안 잡힌다. R&D No.가 없는 기록은 집계에서 빠지기 때문이다.
- **채번 규칙(2026-09-21 확정, `nextStorageNumbers`).** 빈 번호를 남기지 않고 순서대로 나가는 것이 창고팀과의 약속이다. 번호가 왜 비었는지 다른 사람이 알 수 없으면 안 된다.
  ...
  3. 현재 주기 밴드(프론티어에서 아래로, 기록 없는 번호가 20개 이어지면 끊김) 안의 **빈자리를 먼저 메우고**, 없으면 프론티어+1부터 올라간다. 7999를 넘기면 1000부터 되감는다(2026-09-22).
...
- **병합 키 저장은 트랜잭션이다(R240, `firestore-sync.ts` `pushMergedNow`).** 서버의 지금 값을 트랜잭션 안에서 읽어 병합하고 쓴다. 예전처럼 마지막으로 받은 스냅샷(`lastRemote`)과 병합해 `writeBatch`로 통째 덮으면, 팀원이 방금 쓴 항목이 아직 스냅샷으로 안 온 순간 그 항목이 서버에서 빠지고 팀원 화면은 그걸 "원격 삭제"로 읽어 스스로 지운다. 병합 키였던 `records`, `fabricOverrides`, `fabricEvents`도 이 경합에서 잃었다. **병합 키 저장을 `writeBatch`로 되돌리지 말 것.**
  - 병합에 쓰는 내 값과 기준선은 **실행 시점의 화면 값과 기준선을 한 쌍으로** 잡는다. 대기열에서 기다리던 옛 값을 새 기준선과 비교하면 그 사이 팀원이 넣은 항목을 내가 지운 것으로 읽는다. 병합 키 저장은 모두 `setAppState` 직후 같은 값으로 부르므로 화면 값이 곧 최신이다. 새 병합 키 저장 경로도 이 규칙을 지킬 것.
  - `completed`(샘플대장)도 병합 키다. 옛 대장 행 id가 `rnd:번호`라 시트, 주기마다 되풀이되므로(2026-09-22 실측 401그룹 740행) id 하나로 병합하면 겹친 행끼리 지운다. `LIST_MERGE_IDS`가 `시트|id|같은 값 중 몇 번째`로 id를 세운다. 1팀 입고, 웹 등록 id는 고유해야 한다(`f1:시각:순번:난수`, `web:시각:난수`). **이 형식을 바꾸거나 id를 재사용하지 말 것.**
  - 첫 스냅샷 전에는 전송하지 않는다(`initialSync`). 기준선 없이 올리면 이 PC의 옛 캐시가 서버를 흔든다.
  - `records`, `completed`, `fabricOverrides`, `fabricEvents`는 병합 결과가 비면 쓰지 않는다(`NEVER_EMPTY_KEYS`).
  - 전송 실패는 버리지 않고 2, 5, 15, 30, 60초 뒤 지금 화면 값으로 다시 보낸다. `fabric:sync-failed`, `fabric:sync-recovered` 이벤트를 `SyncStatusNotice`(소유자 포함 전원)가 띄운다. 끝내 실패한 저장은 새로 고치면 첫 스냅샷이 로컬을 서버 값으로 바꿔 사라진다. 경고 문구가 그걸 막는 장치다.
  - 병합 키가 아닌 키(`ts`, `study`, `events` 등)는 여전히 마지막 저장이 이긴다. 여러 명이 동시에 쓰는 키를 늘리면 여기 옮길 것.
  - 커밋 한 번의 요청 크기 한도는 10 MiB다. `completed`가 2026-09-22 기준 4.4MB다. 1팀 입고가 쌓이면 언젠가 닿는다.
...
- **캐시 비우기는 동기화 캐시(parsed)만 비우고 새로 고친다(R241, `clearSyncedCache`).** 스토어를 예시 데이터로 바꾼 채 동기화를 켜 두면 병합이 서버 행을 삭제로 읽을 수 있고, `clearCache`는 이 PC에만 있는 화학 첨부까지 지운다. SETTING은 사용자와 권한, 데이터 보호(키별 저장 방식, 크기, 10 MiB 대비, 서버 마지막 저장, 저장 실패), 작업 이력, 연동 네 탭이다. 브라우저에만 저장되고 아무 데서도 읽지 않는 설정 카드를 다시 만들지 말 것.

## 백업 (`src/data/backup-export.ts`)
- JSON은 SETTING 데이터 보호 탭(소유자 전용)에서 내려받고 복원의 원본이다.
