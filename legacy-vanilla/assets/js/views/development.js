...
import { viewHead, badge, kpiRow, button, stageBar, emptyState, card, cols } from '../ui/widgets.js';
...
.development__visual { min-width: 0; }.development__chart { min-height: calc(var(--sp-12) * 5); }.development .view-head__title, .development .card__title { font-family: var(--font-display); letter-spacing: var(--ls-tight); }.development .card { box-shadow: var(--shadow-1); }.development .card:hover { box-shadow: var(--shadow-2); }
...
function visualSummary(records, sub) {
  charts.forEach((chart) => chart.destroy());
  charts = [];
  if (!records.length) return emptyState('시각화할 개발 데이터가 없습니다.');
  const seasons = [...new Set(records.map((record) => record.season).filter(Boolean))].sort();
  const stages = STAGES.map((stage) => ({ label: stage.label, count: records.filter((record) => record.stage === stage.label).length }));
  const seasonHost = el('div.development__chart');
  const stageHost = el('div.development__chart');
  charts.push(createChart(seasonHost, { type: 'bar', height: 230, labels: seasons, datasets: [{ label: `${sub.name} 개발 건`, data: seasons.map((season) => records.filter((record) => record.season === season).length) }] }));
  charts.push(createChart(stageHost, { type: 'doughnut', height: 230, labels: stages.map((item) => item.label), datasets: [{ label: '공정 단계', data: stages.map((item) => item.count) }] }));
  return cols('1-1', [card({ title: '시즌별 개발 포트폴리오', meta: `${records.length}건`, body: seasonHost }), card({ title: '공정 단계 분포', meta: '병목 구간 확인', body: stageHost })]);
}
...
function build(root, store, route) {
  ...
  const visualHost = el('div.development__visual');
  ...
  root.replaceChildren(style, head, kpiHost, visualHost, toolbar, viewHost, panel);
  ...
  function renderResults() {
    ...
    visualHost.replaceChildren(visualSummary(filtered, sub));
