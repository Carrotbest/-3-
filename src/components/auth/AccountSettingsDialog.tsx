...
import { changeOwnName, changeOwnPassword, requestLoginEmailChange, useAuthStore } from "@/data/auth"

type AccountTab = "password" | "email" | "name"
  ...
  const submit = async () => {
    ...
    try {
      if (tab === "password") {
        ...
        await changeOwnName(trimmedName)
        setMessage({ kind: "ok", text: "이름을 바꿨습니다." })
        setNewName(useAuthStore.getState().user?.displayName ?? trimmedName)
      }
      setCurrentPassword("")
    } catch (error) {
    ...
    <DialogContent className="max-w-md">
      ...
      <DialogBody className="space-y-4">
        ...
        <div className="flex gap-1" role="tablist" aria-label="계정 설정 항목">
          ...
          {tabButton("name", "이름 변경", <UserRound className="size-3.5" />)}
        </div>
        ...
        {tab === "email" && isOwner ? <p className="rounded-md bg-[var(--muted)] px-3 py-2 text-xs leading-relaxed text-[var(--muted-foreground)]">
          ...
          {tab !== "name" ? <div className="grid gap-1.5">
          ...
          {tab === "password" ? <>
            ...
            <div className="grid gap-1.5">
              <Label htmlFor="account-new-name">새 이름</Label>
              <Input id="account-new-name" type="text" autoComplete="name" value={newName} onChange={(event) => setNewName(event.target.value)} placeholder={user?.displayName ?? "이름을 입력하세요"} />
            </div>
            <p className="text-[11px] leading-relaxed text-[var(--muted-foreground)]">현재 이름: {user?.displayName?.trim() || "설정되지 않음"}</p>
          </>}
      ...
      <DialogFooter>
        ...
        {tab === "email" && isOwner ? null : <Button type="button" size="sm" disabled={busy || (tab === "password" ? !canSubmitPassword : tab === "email" ? !canSubmitEmail : !canSubmitName)} onClick={() => void submit()}>
          {busy ? <Loader2 className="size-4 animate-spin" /> : null}{tab === "password" ? "비밀번호 변경" : tab === "email" ? "확인 메일 보내기" : "이름 변경"}
        </Button>}
      </DialogFooter>
    </DialogContent>
  </Dialog>
