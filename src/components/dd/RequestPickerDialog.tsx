...
import { linkedLineIds, requestCandidates } from "@/data/request-link"
...
export function RequestPickerDialog({ open, onOpenChange, requests, records, styleNo, initialReqId, onConfirm }: RequestPickerDialogProps) {
  ...
  const candidates = useMemo(
    ...
    [includeLinked, query, records, requests, styleNo],
  )
  const selected = candidates.find(({ style }) => style.reqId === selectedReqId)?.style
    ?? requests.find((style) => style.reqId === selectedReqId)
  ...
  useEffect(() => {
    if (!open || !selected) { setSelectedLineIds(new Set()); return }
    setSelectedLineIds(new Set(selected.options.flatMap((option) => !option.lineId || !linked.has(option.lineId) ? [option.lineId ?? option.optId] : [])))
  }, [linked, open, selectedReqId])
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="w-[92vw] max-w-4xl">
      <DialogHeader><DialogTitle>FABRIC REQUEST에서 불러오기</DialogTitle></DialogHeader>
          ...
          {selected ? <>
            ...
            <div className="max-h-[60vh] space-y-1 overflow-y-auto">
              {selected.options.map((option) => {
                const isLinked = Boolean(option.lineId && linked.has(option.lineId))
                return <label key={option.lineId ?? option.optId} className="flex items-start gap-2 rounded-md px-2 py-1.5 hover:bg-[var(--muted)]">
                  <Checkbox disabled={isLinked} checked={selectedLineIds.has(option.lineId ?? option.optId)} onCheckedChange={(checked) => toggleLine(option.lineId ?? option.optId, checked === true)} />
                  <span className="min-w-0 flex-1"><span className="block truncate text-sm">Opt {option.no} · {option.color} · {option.dyeingMethod}</span><span className="block truncate text-xs text-[var(--muted-foreground)]">{option.yarnDetail || "Yarn Detail 미기재"}</span></span>
                  {isLinked ? <Badge variant="outline">연결됨</Badge> : null}
                </label>
              })}
            </div>
      ...
      <DialogFooter><Button type="button" size="sm" variant="outline" onClick={() => onOpenChange(false)}>취소</Button><Button type="button" size="sm" disabled={!selected || selectedOptions.length === 0} onClick={() => selected && onConfirm(selected, selectedOptions)}>{selectedOptions.length}건 불러오기</Button></DialogFooter>
    </DialogContent>
  </Dialog>
}
