...
import { PerfBadge, usePerformanceIndex } from "@/components/fabric/PerfBadge"
import { lookupPerformance } from "@/data/fabric-performance"
import { applyKeepList, applyListMarks, applyPerformance, applyRddaUsage, buildDisposalItems, buildDisposalWorkbook, DISPOSAL_REASONS, disposalEvent, disposalSummary, disposalWorkbookFileName, formatMeetingPickup, isActiveItem, isCut, isFinalDispose, isKept, itemVerdict, parseMeetingPickup, type DisposalActor, type DisposalCompletionEntry, type DisposalMarkTarget, type DisposalReason } from "@/data/disposal-round"
...
export function DisposalRoundPanel({ ledger, sequenceStart, rounds, actor, canWrite, isOwner, onSave, onCompleteDisposal }: Props) {
  ...
  const applyRddaPerformance = () => {
    if (!selected || !editable || !perfIndex) return
    const result = applyPerformance(selected, perfIndex)
    replaceRound({ ...result.round, history: [...selected.history, disposalEvent(actor, "update", { target: "RDDA 성과", to: `매칭 ${result.matched}건, 오더·베스트 제외 ${result.excluded}건` })] })
    setPerfResult(`RDDA 성과 반영: 매칭 ${result.matched}건, 오더·베스트 원단 제외 ${result.excluded}건`)
  }
  ...
  const preview = useMemo(() => {
    ...
    return from && to ? buildDisposalItems(ledger, from, to, sequenceStart, perfIndex) : []
  }, [draft.from, draft.to, ledger, sequenceStart, perfIndex])
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[var(--radius)] border border-t-4 border-[var(--warning)] bg-[var(--card)] lg:flex-row" onKeyDown={handleKeys} tabIndex={0}>
    ...
    {!selected || !summary ? <div className="grid flex-1 place-items-center p-8 text-sm text-[var(--muted-foreground)]">라운드를 선택하거나 새로 만들어 주세요.</div> : <section className="flex min-h-0 min-w-0 flex-1 flex-col">
      <div className="shrink-0 border-b p-3">
        ...
        {perfResult ? <div className="mt-1 text-xs text-emerald-700">{perfResult}</div> : null}
