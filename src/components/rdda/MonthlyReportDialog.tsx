  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="rdda-print-root h-[96vh] w-[calc(100vw-1rem)] max-w-[96vw] border-white/75 bg-[color-mix(in_oklab,var(--card)_91%,transparent)] backdrop-blur-xl" showCloseButton>
      <div className="rdda-report-editor flex min-h-0 flex-1 flex-col">
        ...
        <DialogBody className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ["바이어 미팅", `${draft.kpi.meetings.toLocaleString()}건`],
              ["제안 원단", `${draft.kpi.offers.toLocaleString()}건`],
              ["전체 픽업률", `${draft.kpi.pickRate.toFixed(1)}%`],
              ["3팀 비중", `${draft.kpi.teamShare.toFixed(1)}%`],
            ].map(([label, value]) => <div key={label} className="relative overflow-hidden rounded-[11px] border border-white/75 bg-white/48 p-4 shadow-[0_12px_26px_-24px_rgba(15,23,42,0.34)]"><span aria-hidden="true" className="absolute inset-x-4 top-0 h-0.5 rounded-full bg-gradient-to-r from-[var(--gradient-1)] to-[var(--gradient-3)]" /><p className="text-[11px] font-medium text-[var(--muted-foreground)]">{label}</p><p className="mt-2 text-[26px] font-semibold tracking-[-0.035em]">{value}</p></div>)}
          </div>
      ...
      <div className="rdda-print-deck" aria-hidden="true">
        ...
        <section className="rdda-print-page"><h2>핵심 지표</h2><div className="rdda-print-kpis">{[["바이어 미팅", `${draft.kpi.meetings.toLocaleString()}건`], ["제안 원단", `${draft.kpi.offers.toLocaleString()}건`], ["전체 픽업률", `${draft.kpi.pickRate.toFixed(1)}%`], ["3팀 비중", `${draft.kpi.teamShare.toFixed(1)}%`]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></section>
