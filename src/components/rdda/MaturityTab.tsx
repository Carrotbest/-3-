...
export function MaturityTab({ report }: { report: RddaReportV2 }) {
  ...
  return <div className="space-y-4">
    <KpiGrid items={[{ label: "가격 구간", value: report.price.length }, { label: "가격 데이터", value: report.price.reduce((sum, row) => sum + row.count, 0).toLocaleString() }, { label: "최고 픽업 구간", value: bestPrice?.band ?? "—", note: `${(bestPrice?.pickRate ?? 0).toFixed(1)}%` }, { label: "최고 성숙 구간", value: bestAge?.band ?? "—", note: `${(bestAge?.hitRate ?? 0).toFixed(1)}%` }]} />
