...
export function RateBar({ value, max = 100, tone = "primary" }: { value: number | null; max?: number; tone?: "primary" | "positive" | "negative" }) {
  ...
  return <div className="relative h-7 min-w-28 overflow-hidden rounded-[7px] border border-white/55 bg-[color-mix(in_oklab,var(--muted)_70%,transparent)] shadow-inner"><span className="absolute inset-y-0 left-0 opacity-[0.16]" style={{ width: `${width * fill}%`, background: `linear-gradient(90deg, transparent, ${color})` }} /><span className="absolute inset-y-0 right-2 flex items-center text-xs font-semibold [font-variant-numeric:tabular-nums]"><AnimatedNumber value={value} decimals={1} suffix="%" fill={fill} /></span></div>
}
