...
/**
 * `embedded`면 자체 제목줄을 그리지 않는다. SETTING의 데이터 탭 안에서 쓰인다.
 * 옛 `/sync` 주소는 SETTING으로 넘어가지만 이 컴포넌트는 그대로 재사용한다.
 */
export function Sync({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return (
    <section className="min-w-0 space-y-6">
      {embedded ? null : <PageHeader title="데이터 상태" subtitle="현재 데이터 출처, 대조 결과와 브라우저 캐시 상태를 보여줍니다." />}
